...
#
# Single image that serves every network transport of the `units` VGI worker:
...
#
# Unlike the Python workers (vgi-scikit-learn et al.) this worker is STATELESS:
# the conversion engine is a curated static table compiled into the binary, so
...
# runtime ships. `git` is preinstalled in the rust image — needed because the
# workspace pins `vgi` via a `[patch.crates-io]` git dependency (see Cargo.toml).
...
RUN --mount=type=cache,target=/usr/local/cargo/registry \
    ...
    cargo build --release --locked --bin units-worker \
    && cp target/release/units-worker /units-worker
...
ARG SOURCE_URL=https://github.com/Query-farm/vgi-units
...
LABEL org.opencontainers.image.title="vgi-units" \
      org.opencontainers.image.description="Runtime physical-unit conversion & dimensional analysis as a VGI worker for DuckDB/SQL (stdio + HTTP + TCP)" \
...
ENV PORT=8000 \
    ...
    VGI_UNITS_GIT_COMMIT=${GIT_COMMIT}
