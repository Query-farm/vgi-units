...
#
# Dispatch the single vgi-units image into one of its transports:
...
#
# The worker is stateless (curated static unit table), so there is no /data to
...
case "${1:-http}" in
  http)
    ...
    exec units-worker --http "$@"
    ;;
  tcp)
    shift 2>/dev/null || true
    exec units-worker --tcp "0.0.0.0:${PORT_TCP:-8001}" "$@"
    ;;
  stdio)
    shift 2>/dev/null || true
    exec units-worker "$@"
